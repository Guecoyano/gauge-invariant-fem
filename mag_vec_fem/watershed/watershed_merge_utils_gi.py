...
def find_minima(Wgrid):
    ...
    print("Storing the x,y coordinates of the minima normalized to a domain of size 1 in x_W.txt, y_W.txt")
...
def construct_network(Wgrid):
    ...
    print("Storing the minima of the boundary between different sub-regions in min_boundary.txt")
...
#         print(ii,jj)
        ...
        for x in range(-1,2):
            for y in range(-1,2):
                if W[index_p(ii+x,n1)][index_p(jj+y,n2)] != i+1 and  W[index_p(ii+x,n1)][index_p(jj+y,n2)] != 0:
                    flag_boundary = 1
                    break
            ...
            if(is_independent(working_list[index], W_ag, min_boundary, neighbors)):
                ...
                for j in neighbors[working_list[index]]:
                    ...
                    if(j not in independent_list):
                        merge_flag = 2
                        # print("j", j)
                        if(merging_condition(working_list[index], j, W_ag, min_boundary)):
                            # print('merging condition verified for', j)
                            merge(working_list[index], j, x_min, y_min, W, min_boundary, neighbors, False)
