...
Functions defined:
    ...
    label_local_minima:
        ...
        equal to 0., except for the strict lower minima set equal to 1., 2., ...
...
class FIFOPriorityQueue(PriorityQueue):
    """
    FIFOPriorityQueue: this is a FIFO priority queue, meaning that items are added to the
    queue with a real number priority, and when items are gotten from the queue, the lower
    priority items go first, with FIFO used to break ties.
...
def verticesdata_frommesh(Th,values=None):
    if values is None:
        val=np.zeros(Th.nq)
    else:
        val=values
...
def label_local_minima(I, mode=None):
    """
    mode irr:
    take an array of vertex class objects and returns an index list with value zero if the vertex is not a 
    local minimum and the ordered index of the local minimum elseways.

    Takes a real 2D array and returns another of the same size which is equal to zero
    except at the strict local minima, which are set to 1., 2., ... with 1. labeling the global
    minimum, 2. labeling the next lowest minimum, etc.  For this purpose a strict local
    minimum is a point with a strictly lower value than its neighbors, so if two contiguous
    points take on exactly the same minimum value, that won't be detected as a minimum.
    This may not be what is wanted.
    
    The parameter mode can be set to 'wrap' to handle periodic functions.  In that case
    we interpret I[i, j] = I[i + k n1, j + l n2] where (n1, n2) = I.shape and k, l are arbitrary integers.
    """
    ...
    if mode == 'irr':
        ...
        for i in range(n1):
            for j in range(n2):
                if I[i, j] < np.min([I[ii, jj] for (ii, jj) in neighbors(i, j, n1, n2, mode=mode)]):
                    J[i, j] = 1.
...
def list_local_minima(J, mode=None):
    """
    Takes a real 2D array generated from the label_local_minima function and returns a list of the 
...
def boundary(I, i, j, mode=None):
    """
    Given a pont i,j and a binary array I with 0 as the values of the contour, finds
    the boundary points of the point.
            ...
            for n in range(len(l_points)):
                ...
                for x in range(-1,2):
                    for y in range(-1,2):    
                        ...
                        elif l_used.count(index(ii+x,Li)+Li*index(jj+y,Lj)) == 0:
...
def region(I, i, j, mode=None):
    """
    Given a pont i,j and a binary array I with 0 as the values of the contour, finds
    the region points of the point.
...
def neighbors(i, j, n1, n2, mode=None):
    """
    Given the pair i, j of indices, with 0 <= i < n1, 0 <= j < n2, returns a numpy array with 2 columns
    and one row for each neighbor, the contents being the indices of the neighbors.  There are 8 neighbors
    except for pixels on the boundary (i = 0 or n1-1 or j = 0 or n2-1).
    
    If mode is set to wrap, periodic wrapping is used and i, j need not belong to [0, n1) and [0, n2), but
    the eight neighbors computed will all have indices belonging to these intervals. 
    """
    if mode == 'wrap':
        i = i % n1
        im1 = (i - 1) % n1
        ...
        jm1 = (j - 1) % n2
        jp1 = (j + 1) % n2
        l = np.array([[im1, jm1], [im1, j], [im1, jp1], [i, jm1], [i, jp1], [ip1, jm1], [ip1, j], [ip1, jp1]])
    else:
        l = np.array([[i-1, j-1], [i-1, j], [i-1, j+1], [i, j-1], [i, j+1], [i+1, j-1], [i+1, j], [i+1, j+1]])
    return l[np.all((l[:, 0 ] > -1, l[:, 0] < n1, l[:, 1] > -1, l[:, 1] < n2), axis=0), :]

def watershed(I, M, mode=None):
    """
    This function defines a watershed transform based on markers and flooding.  This should
    duplicate the Matlab imaging toolbox watershed as long as the markers are the local minima.
    ...
    https://lost-contact.mit.edu/afs/cs.stanford.edu/package/matlab-r2009b/matlab/r2009b/toolbox/images/images/private/mexsrc/watershed_meyer/watershed_meyer.cpp

    Given a 2D real array I and another M where M contains markers, returns a third such
    array which shows the watersheds.  Each element on the output array is either 0
    for points on the watershed boundaries, or 1, 2, 3, ... for points in the watershed
    of marker 1, 2, 3, ...
