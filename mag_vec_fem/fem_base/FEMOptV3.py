...
def KgP1_OptV3_guv(Th,g,dtype):
  ...
  KgElem=lambda il,jl: (factorial(d)/float(factorial(d+3)))*(1+(il==jl))*Th.vols*(gs+gme[:,il]+gme[:,jl])
  for il in range(ndfe):
    for jl in range(ndfe):
...
def KgP1_OptV3_gdudv(Th,g,G,i,j,dtype):
  ...
  gme=gh[Th.me]
  gs=(gme.sum(axis=1)).reshape(Th.nme)
  KgElem=lambda il,jl: (factorial(d)/float(factorial(d+1)))*Th.vols*gs*G[jl,i]*G[il,j]
  for il in range(ndfe):
    for jl in range(ndfe):
      Kg[:,il,jl]=KgElem(il,jl)
  return Kg
...
def KgP1_OptV3_gudv(Th,g,G,i,dtype):
  ...
  gme=gh[Th.me]
  gs=(gme.sum(axis=1)).reshape(Th.nme)
  KgElem=lambda il,jl: (factorial(d)/float(factorial(d+2)))*Th.vols*(gs+gme[:,jl])*G[il,i]
  for il in range(ndfe):
    for jl in range(ndfe):
      Kg[:,il,jl]=KgElem(il,jl)
  return Kg
...
def KgP1_OptV3_gduv(Th,g,G,i,dtype):
  ...
  gme=gh[Th.me]
  gs=(gme.sum(axis=1)).reshape(Th.nme)
  KgElem=lambda il,jl: (factorial(d)/float(factorial(d+2)))*Th.vols*(gs+gme[:,il])*G[jl,i]
  for il in range(ndfe):
    for jl in range(ndfe):
      Kg[:,il,jl]=KgElem(il,jl)
  return Kg
