...
def PlotValOld(Th, u):
    if Th.d == 2:
        ...
        ax.plot_trisurf(
            Th.q[:, 0], Th.q[:, 1], u, triangles=Th.me, cmap=plt.cm.Spectral
        )
        plt.show()
...
def PlotBounds(Th, **kwargs):
    ...
    for i in range(len(LB)):
        ...
        (line,) = ax.plot(
            np.r_[Th.q[Th.be[I[0], 0], 0], Th.q[Th.be[I[0], 1], 0]],
            np.r_[Th.q[Th.be[I[0], 0], 1], Th.q[Th.be[I[0], 1], 1]],
            linewidth=linewidth,
        )
    ...
    for i in range(len(LB)):
        ...
        for k in I:
            (line,) = ax.plot(
                np.r_[Th.q[Th.be[k, 0], 0], Th.q[Th.be[k, 1], 0]],
                np.r_[Th.q[Th.be[k, 0], 1], Th.q[Th.be[k, 1], 1]],
                color=color,
                linewidth=linewidth,
            )
...
def PlotEig(pde, eigenvector, eigenvalue, **kwargs):
    ...
    if abs(eigenvalue.imag) < 1e-12:
        ...
        plt.title(
            r"$\lambda=(%.3f,%.3f)$" % (eigenvalue.real, eigenvalue.imag),
            **titleoptions
        )
    plt.hold(False)
