...
def StiffElasHoperators(d, lam, mu):
    ...
    if isinstance(lam, type(lambda: None)) and np.isscalar(mu):
        if d == 2:
            gam = lambda x1, x2: lam(x1, x2) + 2 * mu
        if d == 3:
            gam = lambda x1, x2, x3: lam(x1, x2, x3) + 2 * mu
    ...
    if isinstance(mu, type(lambda: None)) and isinstance(
        ...
        if d == 3:
            gam = lambda x1, x2, x3: lam(x1, x2, x3) + 2 * mu(x1, x2, x3)
    assert gam != None
    ...
    if d == 3:
        Hop.H[0][0] = Loperator(
            d=d, A=[[gam, None, None], [None, mu, None], [None, None, mu]]
        )
        Hop.H[0][1] = Loperator(
            d=d, A=[[None, lam, None], [mu, None, None], [None, None, None]]
        )
        Hop.H[0][2] = Loperator(
            d=d, A=[[None, None, lam], [None, None, None], [mu, None, None]]
        )
        Hop.H[1][0] = Loperator(
            d=d, A=[[None, mu, None], [lam, None, None], [None, None, None]]
        )
        Hop.H[1][1] = Loperator(
            d=d, A=[[mu, None, None], [None, gam, None], [None, None, mu]]
        )
        Hop.H[1][2] = Loperator(
            d=d, A=[[None, None, None], [None, None, lam], [None, mu, None]]
        )
        Hop.H[2][0] = Loperator(
            d=d, A=[[None, None, mu], [None, None, None], [lam, None, None]]
        )
        Hop.H[2][1] = Loperator(
            d=d, A=[[None, None, None], [None, None, mu], [None, lam, None]]
        )
        Hop.H[2][2] = Loperator(
            d=d, A=[[mu, None, None], [None, mu, None], [None, None, gam]]
        )
    return Hop
